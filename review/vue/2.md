...
# 渐进式框架 Vue
> 一开始不需要掌握全部的特性，可以后续逐步增加功能；
> Vue 主张最少，没有做职责之外的事情
>（1） 简洁 （2） 轻量 （3）快速 （4） 数据驱动 （5） 模块友好 （6） 组件化

## Vue 与 React、Angular 不同的是
1. Vue.js 只提供了 vue-cli 生态中最核心的组件式系统和双向数据绑定
2. 就好像 Vuex、Vue-router 都属于围绕 Vue.js 开发的库
...
### Vue.extend(options)
- Vue.extend 基于 Vue 创建一个子类，参数 options 会作为该子类的默认全局配置，如同 Vue 的默认全局配置一般；
