...
> addEventListener 拓展了第三个参,可为 {} 对象
```js
...
> 阻止默认行为
- `return false`: 没有添加 addEventListener，会阻止默认行为也会**阻止事件冒泡**
- `preventDefault`: 添加了 addEventListener，不会阻止事件冒泡，（或 `returnValue = false`）
...
> splice, sort,reverse等方法会改变原数组，想不改变原数组，就要创建一个副本，以下方法可以不创建副本

#### Array.with(index, value)
```js
const arr = [1, , 'a', 2, 3, ,]
console.log(arr.with(0, 'cc')) // ['cc', undefined, 'a', 2, 3, undefined]
```
...
<!-- 函数嵌套一个函数，内部函数访问到外部函数中的变量，就形成了闭包 -->
...
**xhr.status**
    ...
    200: OK，访问正常
    204: 访问正常，没有任何响应内容（区别于 200响应体为空：在路由导航的情况下，200会使页面展示一个空白文档，204页面无变化）
    206: 访问正常，范围响应，只返回文件的一部分内容 （Partial Content）；若是请求不合法，则返回 416（Range Not Satisfiable）
    ...
    416: Range Not Satisfiable，请求范围响应时，请求的范围不合法
...
## document.domain
`abc.baidu.com` `www.baidu.com` 直接是跨域的，`document.domain=baidu.com` 可以实现二级域名相同的不同域之间的跨域dom访问
...
问题：
- 不能继承父类原型上的属性方法
- 只是调用了父类构造函数上的方法，父类和子类没有关系了（Child instanceof Parent === 'false'）
...
## ES5 和 ES6 继承的区别
- es5 的构造函数可以作为普通函数调用，es6 class 不能
- ES5的继承，实质是先创造子类的实例对象this，然后再执行父类的构造函数给它添加实例方法和属性(不执行也无所谓）。而ES6的继承机制完全不同，实质是先创造父类的实例对象this（当然它的__proto__指向的是子类的prototype），然后再用子类的构造函数修改this
- es6 可以继承原生构造函数如 Number 


[JS 面向对象之继承](https://juejin.cn/post/6959528751445049352?from=search-suggest#heading-12)
...
成功的时候返回的是一个结果数组，而`失败的时候则返回最先被reject失败状态的值`


promise.all 的问题：
    一旦某个 promise 被 reject 了，只会返回失败状态的值，其他成功状态的值都不返回，
    要么全部成功，要么全部重来
...
**和 ==/=== 运算符不同**，== 运算符会对它两边的操作数做*隐式类型转换*（如果它们类型不同），然后才进行相等性比较，所以会导致 "" == false
...
根据浏览器刷新频率，在每次渲染前执行回调函数更新动画，优点是动画流畅

不同设备的浏览器刷新频率不同，所以执行频率也不同
...
但是 setTimeout 与 setInterval 这些定时器的时间设定可能不准，requestAnimationFrame 则是根据浏览器的刷新频率来的

用处：
- 大量数据渲染如 10w+
requestAnimationFrame + Fragment文档碎片 （虚拟的 DOM 节点容器，可以在其中存储多个 DOM 元素，但不会直接在页面中渲染显示，在内存中构建一组 DOM 元素，然后一次性将它们添加到文档中，从而减少 DOM 操作的重绘和重排，提高性能）


## requestIdleCallback

在浏览器渲染的每一帧中，利用剩余时间（如果有）（执行完用户输入、布局绘制等任务）后，执行某个任务

- 不一定每一帧都有空闲时间、都能执行
- 主要用于执行一些比较耗时，但又不是很重要很紧急的任务
- 不要在 requestIdleCallback 中修改DOM
...
2. 响应头中设置了 `Content-Disposition` 为 `attachment`
```http
Content-Disposition: attachment; filename="filename.ext"
```

3. MIME类型设置为 `application/octet-stream`
