...
let oH4 = document.getElementById('h4')
...
执行顺序：
    事件捕获 -> 事件冒泡
    从捕获，向子节点查找事件，顺序为 `div 冒泡 -> oH4`

**同一个节点设置冒泡和不冒泡**，则根据书写顺序来：`div 冒泡 -> div 不冒泡`

上述代码顺序：`div 冒泡 -> h4 -> div 不冒泡`
...
**规则如下：**  
    ...
    - 字符串中不能单独出现双引号(") 和 右斜杠("\"),如需使用，需��用、进行转义如：
    \" 和 \\ 其他需转义的也如此  
